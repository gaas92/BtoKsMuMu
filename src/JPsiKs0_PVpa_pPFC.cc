// -*- C++ -*-
//
// Package:    JPsiKs0_PVpa_pPFC
// Class:      JPsiKs0_PVpa_pPFC
// 
//=================================================
...
// user include files
#include "myAnalyzers/BtoKsMuMu/src/JPsiKs0_PVpa_pPFC.h"
...
//
JPsiKs0_PVpa_pPFC::JPsiKs0_PVpa_pPFC(const edm::ParameterSet& iConfig)
  :
...
JPsiKs0_PVpa_pPFC::~JPsiKs0_PVpa_pPFC()
{

}

// ------------ method called to for each event  ------------
void JPsiKs0_PVpa_pPFC::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using std::vector;
  using namespace edm;
...
bool JPsiKs0_PVpa_pPFC::IsTheSame(const reco::Track& tk, const pat::Muon& mu){
  double DeltaEta = fabs(mu.eta()-tk.eta());
  double DeltaP   = fabs(mu.p()-tk.p());
  if (DeltaEta < 0.02 && DeltaP < 0.02) return true;
  return false;
}
bool JPsiKs0_PVpa_pPFC::IsTheSame(const pat::GenericParticle& tk, const pat::Muon& mu){
  double DeltaEta = fabs(mu.eta()-tk.eta());
  double DeltaP   = fabs(mu.p()-tk.p());
  if (DeltaEta < 0.02 && DeltaP < 0.02) return true;
  return false;
}
bool JPsiKs0_PVpa_pPFC::IsTheSame(const pat::GenericParticle& tk1, const pat::GenericParticle& tk2){
  double DeltaEta = fabs(tk2.eta()-tk1.eta());
  double DeltaP   = fabs(tk2.p()-tk1.p());
  if (DeltaEta < 0.02 && DeltaP < 0.02) return true;
  return false;
}
bool JPsiKs0_PVpa_pPFC::isAncestor(const reco::Candidate* ancestor, const reco::Candidate * particle) {
...
bool JPsiKs0_PVpa_pPFC::isAncestor(int a_pdgId, const reco::Candidate * particle) {
...
double JPsiKs0_PVpa_pPFC::GetLifetime(TLorentzVector b_p4, TVector3 production_vtx, TVector3 decay_vtx) {
...
std::string JPsiKs0_PVpa_pPFC::printName(int pdgid){
...
void JPsiKs0_PVpa_pPFC::printMCtree(const reco::Candidate* mother, int indent=0){
    if (mother == NULL){
...
void JPsiKs0_PVpa_pPFC::printMCtreeUP(const reco::Candidate* daughter, int indent = 0){
    if (daughter == NULL){
        std::cout << "end tree" << std::endl;
        return;
    }
...
void JPsiKs0_PVpa_pPFC::endJob() {
  tree_->GetDirectory()->cd();
  tree_->Write();
}

//define this as a plug-in
DEFINE_FWK_MODULE(JPsiKs0_PVpa_pPFC);
